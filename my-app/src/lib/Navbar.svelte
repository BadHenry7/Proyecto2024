<script>
  ...
  let error = null;
  onMount(() => {
    //no va a funcionar  no no no no no no dale a seguirme
  ...
  async function mostrar_modulos() {
    ...
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/get_modulos_asignado",
        {
          method: "POST",
          headers: {
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarNavDropdown">
              ...
              <ul class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                {#each todos as subtodo}
                  ...
                  {#if subtodo.nombre_modulo === todo.nombre_modulo}
                    ...
                    <li>  
                      <a class="dropdown-item" href={subtodo.url}
                        >{subtodo.submodulo}</a
                      >
                    </li>
                  {/if}
      ...
      <form class="d-flex nav-item nav-link ms-auto">
        <button
          class="btn btn-outline-dark nav-item"
          on:click={confirmacion}
          href="#"
          aria-label="Cerrar edición de usuario">Cerrar sesion</button
        >
      </form>
    </div>
  </div>
